...
class BaseStorage(ABC):
    ...
    @abstractmethod
    def list_recent_exams(self) -> list[str]:
        """List distinct exam IDs available in storage."""

    @abstractmethod
    def delete_exam(self, exam_id: str) -> None:
        """Delete all records and files associated with an exam ID."""
...
class LocalStorage(BaseStorage):
    ...
    def list_recent_exams(self) -> list[str]:
        records = self._read_json(self._grades_path)
        exams = set(r["exam_id"] for r in records if "exam_id" in r)
        return list(exams)

    def delete_exam(self, exam_id: str) -> None:
        grades = self._read_json(self._grades_path)
        grades = [g for g in grades if g.get("exam_id") != exam_id]
        self._write_json(self._grades_path, grades)

        ocr_records = self._read_json(self._ocr_path)
        ocr_records = [o for o in ocr_records if o.get("exam_id") != exam_id]
        self._write_json(self._ocr_path, ocr_records)

        exam_pdf_dir = self.pdfs / exam_id
        if exam_pdf_dir.exists() and exam_pdf_dir.is_dir():
            import shutil
            shutil.rmtree(exam_pdf_dir)
...
class S3Storage(BaseStorage):
    ...
    def list_recent_exams(self) -> list[str]:
        # Return empty for now; full Dynamo query omitted for brevity.
        return []

    def delete_exam(self, exam_id: str) -> None:
        raise NotImplementedError("delete_exam not implemented for S3")
