 ...
 * Responsive layout: sidebar on md+, collapsible topbar on mobile.
 */

import { useState } from "react";
import { NavLink, useNavigate } from "react-router-dom";
import {
  LayoutDashboard, Upload, CheckSquare, BarChart2, Activity,
  Menu, X, GraduationCap, Search, User, ChevronRight,
} from "lucide-react";
    ...
    <nav aria-label="Main navigation" className="flex-1 space-y-0.5">
      {NAV.map(({ to, icon: Icon, label }) => (
        ...
        >
          ...
          <span>{label}</span>
          {/* Active indicator */}
          <ChevronRight className="w-3.5 h-3.5 ml-auto opacity-0 group-[.active]:opacity-100 text-blue-500" />
        </NavLink>
      ))}
    </nav>
  );
...
export default function Layout({ children }) {
  ...
  const handleSearch = (e) => {
    ...
    if (!v) return;
    // Jump to job status if looks like job id, else go to review with exam id
    if (v.startsWith("job_") || v.startsWith("JOB")) {
      navigate(`/jobs/${v}`);
    } else {
      navigate(`/review?exam_id=${v}`);
    }
    setSearchVal("");
  };
        ...
        <form onSubmit={handleSearch} className="px-1 mb-4">
          <div className="relative">
            ...
            <input
              ...
              placeholder="Jump to exam / job ID…"
              value={searchVal}
              onChange={(e) => setSearchVal(e.target.value)}
              aria-label="Jump to exam or job"
            />
          </div>
        </form>
        ...
        <div className="mt-auto pt-4 border-t border-slate-100">
          <div className="flex items-center gap-2.5 px-3 py-2 rounded-lg">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-semibold text-slate-700 truncate">TA User</p>
              <p className="text-[10px] text-slate-400 truncate">ta@university.edu</p>
            </div>
          </div>
        </div>
      </aside>
