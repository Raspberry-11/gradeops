...
Usage:
    agent = GradingAgent(llm_provider="gemini", model_name="gemini-2.0-flash")
...
def _build_criterion_prompt(
    ...
    kw = ", ".join(
        criterion.required_keywords) if criterion.required_keywords else "none"
    return (
        f"STUDENT ANSWER:\n{answer}\n\n"
...
def make_grade_criterion_node(llm):
    ...
    def grade_criterion(state: GradingState) -> GradingState:
        ...
        try:
            ...
            logger.error("LLM grading error on criterion %s: %s",
                         criterion.criterion_id, exc)
...
class GradingAgent:
    ...
    def grade_batch(
        ...
        for ocr in ocr_results:
            ...
            if rubric is None:
                logger.warning(
                    "No rubric for question %d — skipping", ocr.question_number)
                continue
